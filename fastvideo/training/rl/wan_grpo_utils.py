...
def get_fsdp_mesh_from_model(model: torch.nn.Module) -> Optional[torch.distributed.DeviceMesh]:
    """
    Extract the device mesh from an FSDP-wrapped model by inspecting its parameters.

    Returns None if the model is not FSDP-wrapped or no DTensor parameters are found.
    """
...
def tensor_to_dtensor(
    ...
    """
    Convert a regular torch.Tensor to DTensor with Replicate placement.

    This is necessary when passing inputs to an FSDP-wrapped model that has DTensor
    parameters, as PyTorch requires all operands to be either Tensor or DTensor.

    Args:
        tensor: Regular torch.Tensor to convert
        device_mesh: DeviceMesh from the FSDP-wrapped model

    Returns:
        DTensor with Replicate placement on all mesh dimensions
    """
    if isinstance(tensor, DTensor):
        return tensor

    # Create Replicate placement for each dimension of the mesh
    num_dims = device_mesh.ndim
    placements = tuple(Replicate() for _ in range(num_dims))

    return DTensor.from_local(tensor, device_mesh=device_mesh, placements=placements)


def dtensor_to_tensor(tensor: Union[torch.Tensor, DTensor]) -> torch.Tensor:
    """
    Convert a DTensor back to a regular torch.Tensor.

    Args:
        tensor: DTensor or regular tensor

    Returns:
        Regular torch.Tensor (local tensor if DTensor)
    """
    if isinstance(tensor, DTensor):
...
def wan_pipeline_with_logprob(
    ...
    # Check if transformer is FSDP-wrapped by looking for DTensor parameters
    # If so, we need to convert inputs to DTensor with Replicate placement
    fsdp_mesh = get_fsdp_mesh_from_model(transformer)
    is_fsdp_model = fsdp_mesh is not None
    if is_fsdp_model:
        logger.info("Transformer is FSDP-wrapped, inputs will be converted to DTensor")
        logger.info("FSDP mesh: %s", fsdp_mesh)
    else:
        logger.info("Transformer is not FSDP-wrapped, using regular tensors")
    ...
    # Convert prompt embeddings to DTensor if model is FSDP-wrapped
    if is_fsdp_model:
        prompt_embeds = tensor_to_dtensor(prompt_embeds, fsdp_mesh)
        if negative_prompt_embeds is not None:
            negative_prompt_embeds = tensor_to_dtensor(negative_prompt_embeds, fsdp_mesh)
        logger.info("Converted prompt_embeds to DTensor")
    ...
    # Debug
    logger.info(f"Tensor type after DTensor conversion:")
    logger.info(f"latents: {type(latents)}")
    logger.info(f"prompt_embeds: {type(prompt_embeds)}")
    if is_fsdp_model:
        logger.info(f"prompt_embeds placement: {prompt_embeds.placements}")
    ...
    for i, t in enumerate(timesteps):
        ...
        # Print FSDP args only on first iteration
        if i == 0:
        ...
        # Convert inputs to DTensor if model is FSDP-wrapped
        if is_fsdp_model:
            latent_model_input = tensor_to_dtensor(latent_model_input, fsdp_mesh)
            timestep = tensor_to_dtensor(timestep, fsdp_mesh)
        ...
        # Convert output back to regular tensor if needed
        noise_pred = dtensor_to_tensor(noise_pred)
        noise_pred = noise_pred.to(dtensor_to_tensor(prompt_embeds).dtype)
        ...
        if do_classifier_free_guidance:
            ...
            noise_uncond = dtensor_to_tensor(noise_uncond)
        ...
        if kl_reward > 0 and not determistic:
            ...
            # Convert to DTensor if needed
            if is_fsdp_model:
                latent_model_input_ref = tensor_to_dtensor(latent_model_input_ref, fsdp_mesh)
            ...
            # Convert output back to regular tensor
            noise_pred_ref = dtensor_to_tensor(noise_pred_ref)
            noise_pred_ref = noise_pred_ref.to(dtensor_to_tensor(prompt_embeds).dtype)
