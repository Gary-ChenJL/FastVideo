...
from fastvideo.training.rl.wan_grpo_utils import (
    wan_pipeline_with_logprob,
    sde_step_with_logprob,
    get_fsdp_mesh_from_model,
    tensor_to_dtensor,
    dtensor_to_tensor,
)
...
class RLPipeline(TrainingPipeline):
    ...
    def _compute_log_prob_for_timestep(
        ...
        # Check if transformer is FSDP-wrapped
        ...
        if guidance_scale > 1.0 and negative_prompt_embeds is not None:
            ...
            latent_model_input_cfg = torch.cat([dtensor_to_tensor(latent_model_input)] * 2)
            prompt_embeds_cfg = torch.cat([dtensor_to_tensor(negative_prompt_embeds), dtensor_to_tensor(prompt_embeds)])
            timestep_cfg = dtensor_to_tensor(timestep).repeat(2)

            # Re-convert to DTensor if needed
            if is_fsdp_model:
                latent_model_input_cfg = tensor_to_dtensor(latent_model_input_cfg, fsdp_mesh)
                prompt_embeds_cfg = tensor_to_dtensor(prompt_embeds_cfg, fsdp_mesh)
                timestep_cfg = tensor_to_dtensor(timestep_cfg, fsdp_mesh)
