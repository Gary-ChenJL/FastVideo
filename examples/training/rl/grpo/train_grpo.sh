...
#
# This script trains a video generation model using GRPO with GRPO-Guard safety mechanisms.
# Supports single-GPU and multi-GPU training via torchrun.
#
# Usage:
#   Single GPU:  NUM_GPUS=1 ./train_grpo.sh
#   Multi GPU:   NUM_GPUS=4 ./train_grpo.sh
#   8 GPU:       NUM_GPUS=8 ./train_grpo.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-checkpoints/wan_grpo_training}"
...
# Training arguments
training_args=(
    --tracker_project_name "wan_grpo_training"
...
rl_args=(
    ...
    # Training schedule
    --rl_warmup_steps 500
    # Reward configuration (use dummy for testing, replace with actual reward models)
